...
public class BuyerServiceImplV1 implements BuyerService{
	...
	protected BuyerDto result2Dto(ResultSet result) {
		...
		try {
			buyerDto.buId = result.getString(DBContract.BIYER.BUID);
			buyerDto.buName = result.getString(DBContract.BIYER.BUNAME);
			buyerDto.buTel = result.getString(DBContract.BIYER.BUTEL);
			buyerDto.buAddr = result.getString(DBContract.BIYER.BUADDR);
			buyerDto.buBirth = result.getString(DBContract.BIYER.BUBIRTH);
			buyerDto.buJob = result.getString(DBContract.BIYER.BUJOB);
		} catch (SQLException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	public int insert(BuyerDto dto) {
		...
		try {
			...
			// sql 로 전달하는 명령대로 DB 를 변경(insert) 하라 는 의미
